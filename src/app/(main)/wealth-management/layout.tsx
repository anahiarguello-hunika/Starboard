

'use client';

import {
    Briefcase,
    Building,
    DollarSign,
    Users,
    BrainCircuit,
    Package,
    FileText,
    ShieldCheck,
    Landmark,
    Server,
    Scale,
    FileCheck,
    FileType,
    Book,
    MoreHorizontal,
    LayoutDashboard,
} from "lucide-react";
import Link from 'next/link';
import { usePathname } from 'next/navigation';
import { cn } from "@/lib/utils";
import React from "react";

const wealthManagementNav = [
    { name: 'Dashboard', icon: LayoutDashboard, href: "/wealth-management/dashboard"},
    { name: 'Proyectos (Matters)', icon: Briefcase, href: "#"},
    { 
        name: 'Inversiones en Sociedades', 
        icon: Building, 
        href: "/wealth-management/corporate-information",
    },
    { 
        name: 'Información Financiera', 
        icon: DollarSign, 
        href: "/wealth-management/financial-information",
    },
    { 
        name: 'Empleados', 
        icon: Users, 
        href: "/wealth-management/laboral",
    },
    { 
        name: 'Marcas y Propiedad Intelectual', 
        icon: BrainCircuit, 
        href: "/wealth-management/intellectual-property",
    },
    { 
        name: 'Activos', 
        icon: Package, 
        href: "/wealth-management/assets",
    },
    { name: 'Contratos', 
        icon: FileText, 
        href: "/wealth-management/contracts",
    },
    { 
        name: 'Cumplimiento Legal', 
        icon: ShieldCheck, 
        href: "/wealth-management/legal-compliance",
    },
    { 
        name: 'Seguros', 
        icon: Landmark, 
        href: "/wealth-management/insurance",
    },
    { 
        name: 'Sistemas', 
        icon: Server, 
        href: "/wealth-management/systems-and-it",
    },
    { name: 'Litigios', 
        icon: Scale, 
        href: "/wealth-management/litigation",
    },
    { name: 'Concesiones', icon: FileCheck, href: "/wealth-management/concessions" },
    { name: 'Formatos', icon: FileType, href: "/wealth-management/formats" },
    { 
        name: 'Miscelaneos', 
        icon: Book, 
        href: "/wealth-management/miscellaneous",
    },
    { 
        name: 'Metodologías', 
        icon: FileText, 
        href: "/wealth-management/methodologies",
    },
    { name: 'Otros', icon: MoreHorizontal, href: "/wealth-management/others" },
];

export default function WealthManagementLayout({
  children,
}: {
  children: React.ReactNode;
}) {
  const pathname = usePathname();

  return (
    <div className="grid grid-cols-[280px_1fr] gap-8 items-start">
        <div className="flex flex-col gap-4">
             <nav className="flex flex-col gap-1 text-sm text-muted-foreground">
                <div className="flex flex-col gap-1 mt-2">
                {wealthManagementNav.map((item, index) => (
                    <Link key={item.name} href={item.href || "#"} className={cn(
                        'flex items-center gap-3 px-3 py-2 rounded-lg transition-colors',
                        pathname === item.href ? 'bg-primary/10 text-primary font-semibold' : 'hover:bg-muted'
                    )}>
                        <span className="text-xs w-6 text-right">{String(index+1).padStart(2, '0')}</span>
                        <item.icon className="h-5 w-5" />
                        <span>{item.name}</span>
                    </Link>
                ))}
                </div>
            </nav>
        </div>
        <div className="flex flex-col gap-8">
            {children}
        </div>
    </div>
  );
}
